...
class DoubleDQNAgent:
    def __init__(self, input_size, output_size, hidden_size, alpha, epsilon, gamma):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.Q_network1 = QNetwork(input_size, hidden_size, output_size).to(self.device)
        self.Q_network2 = QNetwork(input_size, hidden_size, output_size).to(self.device)
    ...
    def choose_action(self, state, available_moves):
        ...
        best_move_index = np.argmax(Q_values_target)
    ...
    def update_Q_values(self, state, action, reward, next_state):
        ...
        # Convert Q1_values and target to 1D tensors
        Q1_values = Q1_values.view(-1)
...
def train(num_episodes, alpha, epsilon, gamma):
    ...
    gamma = gamma
    agent = DoubleDQNAgent(input_size, output_size, hidden_size, alpha, gamma, epsilon)
